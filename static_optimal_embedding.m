...
S = size(new_requests, 1);
P = size(previous_requests, 1);

if(size(previous_requests, 1) ~= 0)
    total_requests = [previous_requests; new_requests];
else
...
% Exclude exisiting networks - constraint on a

C = zeros(size(existing_networks, 1), b_offset*2);
D = zeros(size(existing_networks, 1), 1);
...
for k=1:P+S
    ...
    for i=1:DF
        for j=1:DT
            if(substrate(i, j) == total_requests(k, 7))
                for m =1:size(existing_networks, 1)
                    if(total_requests(k, 7) == existing_networks(m, 7))
                        if(b_check ==1)
                            b_check=0;
                            C(k, b_offset + j +(i-1)*DT+(k-1)*DF*DT) = existing_networks(m, 2);
                            D(k, 1) = existing_networks(m, 2);
                        end
                    end
                end
            end
...
C2 = zeros(P, b_offset*2);
for i=0:P-1
    C2(i+1,b_offset +1+i*DF*DT: b_offset +DF*DT+i*DF*DT) = 1;
end
D2 = ones(P, 1);
...
for i=1:P+S
    ...
    for j=1:DF
        for k=1:DT
            if(result((i-1)*DF*DT + (j-1)*DT +k, 1) ~= 0)
